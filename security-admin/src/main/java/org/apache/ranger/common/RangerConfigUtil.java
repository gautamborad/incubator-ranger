...
public class RangerConfigUtil {
	...
	public RangerConfigUtil() {
		...
		// defaultMaxRows =
		// PropertiesUtil.getIntProperty("xa.db.maxrows.default",defaultMaxRows);/
		defaultMaxRows = PropertiesUtil.getIntProperty(
				"ranger.db.maxrows.default", defaultMaxRows);
		// roles = PropertiesUtil.getPropertyStringList("xa.users.roles.list");
		roles = PropertiesUtil.getPropertyStringList("ranger.users.roles.list");

//		accessFilterEnabled = PropertiesUtil.getBooleanProperty("xa.db.access.filter.enable", true);
//		isModerationEnabled = PropertiesUtil.getBooleanProperty("xa.moderation.enabled", isModerationEnabled);
//		isUserPrefEnabled = PropertiesUtil.getBooleanProperty("xa.userpref.enabled", isUserPrefEnabled);
